...
// nav.js už dřív poslouchal SSE event 'achievementUpdate' (bell toast), ale
// nikde se neemitoval — grant() teď přes tenhle emitter pošle server.js
// signál, aby ho přeposlal přes broadcastSSE (viz server.js).
...
function grant(userId, key, udelil) {
  ...
  events.emit('granted', { userId, key, label: info.label, icName: radek?.ic_name || 'Neznámý', manual: !!udelil });
