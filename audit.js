...
function renderAudit(req) {
  ...
  <main>
    ...
    <p class="folio-footnote"><strong>Audit.</strong> Chronologicky seřazené záznamy všech akcí v systému — vklady, výběry, finanční pohyby i jejich autoři. Filtruj podle sekce nebo hledej textem. <strong>Reserve Fond</strong> je samostatný bankovní účet organizace — jeho pohyby jsou vidět zde, ale nepočítají se do souhrnu hlavního účtu níže.</p>

    <div id="ucet-souhrn-wrap" style="display:none;margin-bottom:2rem">
      <div class="folio-label" style="margin-bottom:1rem">Účetnictví (hlavní účet) — souhrn per člen</div>
    ...
    <div class="card">
      ...
      <div style="display:flex;gap:0.4rem;margin-bottom:1.5rem;flex-wrap:wrap;align-items:center">
        ...
        <button class="typ-btn" onclick="filterAudit('Reserve Fond')" id="filter-rezerva" style="flex:none;padding:0.4rem 1rem">Reserve Fond</button>
  ...
  <script>
    ...
    function renderTable(events){
      ...
      const SEKCE={
        ...
        'Reserve Fond':{letter:'🏦',color:'#8FD3FF'},
      };
    ...
    function filterAudit(sekce){
      ...
      const map={'vse':'filter-vse','Zbraně':'filter-zbrane','Weed':'filter-weed','Drogy':'filter-drogy','Chemky':'filter-chemky','Účetnictví':'filter-ucet','Reserve Fond':'filter-rezerva'};
    ...
    ['skladUpdate','ucetUpdate','reserveFundUpdate'].forEach(ev=>evtSrc.addEventListener(ev,()=>setTimeout(loadAudit,2000)));
  </script>
  </body></html>`;
}
